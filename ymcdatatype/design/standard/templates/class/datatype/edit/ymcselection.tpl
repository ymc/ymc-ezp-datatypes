...
{* start added id=.. by ymc-flfe: buttons for sorting options need an identifier *}
<table id="id_{$class_attribute.id}" class="list" cellspacing="0">
{* end added id=.. by ymc-flfe *}
<tr>
    <th class="tight">&nbsp;</th>
...
<tr>
    ...
    {* Option. *}
    {* start changed class=.. by ymc-flfe: buttons for sorting options need an identifier class *}
    <td><input class="box box_to_sort" type="text" name="ContentClass_ezselection_option_name_array_{$class_attribute.id}[{$Option:item.id}]" value="{$Option:item.name}" /></td>
    {* end changed class=.. by ymc-flfe *}
</tr>
{/section}
</table>
{section-else}
...
{* start by ymc-flfe: buttons for sorting options via JavaScript *}
<input class="button" type="button" name="sortInputFields" value="Alphabetisch sortieren" title="Optionsfelder alphabetisch sortieren" onclick="sort_fields('id_{$class_attribute.id}', 'box box_to_sort');" />
{* end by ymc-flfe *}
</fieldset>
</div>
